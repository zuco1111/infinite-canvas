...
import { ModelPicker } from '@/features/settings';
import {
  defaultConfig,
  useAppTheme,
  useEffectiveConfig,
...
export function CanvasConfigNodePanel({
  ...
  const canGenerate =
    hasComposerContent || (mode === 'audio' ? inputSummary.textCount > 0 : hasAnyInput);
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-center justify-between gap-3">
        ...
        <div className="cursor-default" onMouseDown={(event) => event.stopPropagation()}>
          <Segmented
            ...
            value={mode}
            onChange={(value) =>
              onConfigChange(node.id, { generationMode: value as CanvasGenerationMode })
            }
            options={[
              {
                value: 'image',
      ...
      <div
        className={`mb-2 grid min-w-0 cursor-default items-center gap-2 ${mode === 'image' || mode === 'video' || mode === 'audio' ? 'grid-cols-[minmax(0,1fr)_148px]' : 'grid-cols-1'}`}
        onMouseDown={(event) => event.stopPropagation()}
      >
        <ModelPicker
        ...
        {mode === 'video' ? (
          ...
          <CanvasImageSettingsPopover
            ...
            onConfigChange={(key, value) =>
              onConfigChange(
                node.id,
                key === 'count' ? { count: Number(value) || 1 } : { [key]: value },
              )
            }
          />
        ) : mode === 'audio' ? (
...
function buildNodeConfig(
  ...
  return {
    ...
    videoWatermark:
      node.metadata?.watermark || globalConfig.videoWatermark || defaultConfig.videoWatermark,
    audioVoice: node.metadata?.audioVoice || globalConfig.audioVoice || defaultConfig.audioVoice,
    audioFormat:
      node.metadata?.audioFormat || globalConfig.audioFormat || defaultConfig.audioFormat,
