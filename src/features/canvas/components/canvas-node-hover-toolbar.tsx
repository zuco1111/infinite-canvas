...
import { formatBytes, getDataUrlByteSize } from '@/shared/media/image-utils';
...
export function CanvasNodeHoverToolbar({
  ...
  const [quickImageToolIds, setQuickImageToolIds] =
    useState<ImageQuickToolId[]>(defaultImageQuickToolIds);
  const [showImageToolLabels, setShowImageToolLabels] = useState(true);
  const [draftImageToolIds, setDraftImageToolIds] =
    useState<ImageQuickToolId[]>(defaultImageQuickToolIds);
  ...
  const nodeToolbarTools: ToolbarTool[] = [
    ...
    ...(isAudio
      ? [
          {
            id: 'uploadAudio',
            title: hasAudio ? '替换音频' : '上传音频',
            label: hasAudio ? '替换音频' : '上传音频',
...
export function CanvasNodeInfoModal({
  ...
  const imageBytes =
    node?.type === CanvasNodeType.Image && node.metadata?.content
      ...
      : 0;
  const batchCount =
    node?.type === CanvasNodeType.Image ? node.metadata?.batchChildIds?.length || 0 : 0;
  const json = useMemo(() => {
    if (!node) return '';
