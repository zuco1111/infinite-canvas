...
export const CanvasResourceMentionTextarea = forwardRef<HTMLTextAreaElement, Props>(
  function CanvasResourceMentionTextarea(
    ...
    const candidates = useMemo(() => {
      ...
      return activeReferences.filter((item) =>
        `${item.label} ${item.title} ${item.kind} ${item.text || ''}`.toLowerCase().includes(query),
      );
    }, [mention, references]);
    const activeLabels = useMemo(
      () =>
        highlightLabels
          ? Array.from(
              new Set(references.filter((item) => item.active).map((item) => item.label)),
      ...
      <div className={`relative h-full w-full ${containerClassName || ''}`}>
        {showOverlay ? (
          <div
            ...
            style={{ ...style, zIndex: 0, color: theme.node.text }}
          >
...
function MentionMenu({
  ...
  const boundary = textarea.closest('.ant-modal-content')?.getBoundingClientRect() || {
    left: 8,
    top: 8,
  ...
  const showAbove =
    rect.bottom + gap + maxMenuHeight > boundary.bottom &&
    rect.top - gap - maxMenuHeight >= boundary.top;
  const top = clamp(
    showAbove ? rect.top - gap - maxMenuHeight : rect.bottom + gap,
    boundary.top + 8,
...
function ReferencePreview({ reference }: { reference: CanvasResourceReference }) {
  if (reference.kind === 'image' && reference.previewUrl)
    return <img src={reference.previewUrl} alt="" className="size-9 rounded-md object-cover" />;
