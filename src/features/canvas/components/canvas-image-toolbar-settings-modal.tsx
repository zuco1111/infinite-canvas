...
export function ImageToolSettingsModal({
  ...
  useEffect(() => {
    ...
    const resizeObserver =
      typeof ResizeObserver !== 'undefined' && toolbar ? new ResizeObserver(sync) : null;
    if (toolbar) {
  ...
  const scrollbarThumbWidth =
    previewScroll.max > 0
      ? Math.min(
          scrollbarWidth,
          Math.max(64, (previewScroll.viewport / previewScroll.content) * scrollbarWidth),
        )
      : scrollbarWidth;
      ...
      >
        <div className="relative flex min-h-[300px] w-full justify-center pt-20 pb-9">
          ...
          <input
            ...
            style={
              { '--preview-scrollbar-thumb-width': `${scrollbarThumbWidth}px` } as CSSProperties
            }
