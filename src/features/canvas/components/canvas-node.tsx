...
import { formatBytes } from '@/shared/media/image-utils';
...
export const CanvasNode = React.memo(function CanvasNode({
  ...
  const isBatchRoot =
    data.type === CanvasNodeType.Image && Boolean(data.metadata?.isBatchRoot) && batchCount > 1;
  ...
  return (
    ...
    >
      ...
      {showPanel && renderPanel ? (
        <div className="absolute left-1/2 top-full z-[70] w-[500px] -translate-x-1/2 pt-4">
          {renderPanel(data)}
        </div>
      ) : null}
    </div>
      ...
      {isEditingContent ? (
        ...
        >
          {node.metadata?.content || (
            <span style={{ color: theme.node.placeholder }}>双击编辑文字</span>
          )}
        </div>
      )}
    </div>
...
function ResourceLabelBadge({ reference }: { reference: CanvasResourceReference }) {
  return (
    <span
      className={`pointer-events-none absolute right-2 top-2 z-30 rounded-md px-1.5 py-0.5 text-[10px] font-medium ${reference.active ? 'bg-[#2f80ff] text-white shadow-sm' : 'bg-black/35 text-white/75'}`}
    >
      {reference.label}
    </span>
  );
...
function ImageContent({
  ...
  return (
    <BatchFrame
      batchCount={isBatchRoot ? batchCount : 0}
      batchExpanded={batchExpanded}
      batchOpening={batchOpening}
      batchRecovering={batchRecovering}
      onToggleBatch={onToggleBatch}
