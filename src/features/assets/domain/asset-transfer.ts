...
import { createZip, readZip } from '@/shared/storage/zip';
import { getMediaBlob, setMediaBlob } from '@/shared/storage/file-storage';
import { getImageBlob, setImageBlob } from '@/shared/storage/image-storage';
...
export async function exportAssets(assets: Asset[]) {
  ...
  await Promise.all(
    assets.map(async (asset) => {
      ...
      const blob =
        asset.kind === 'image' ? await getImageBlob(storageKey) : await getMediaBlob(storageKey);
      if (!blob) return;
  ...
  const zip = await createZip([
    { name: 'assets.json', data: JSON.stringify(data, null, 2) },
    ...zipFiles,
  ]);
  saveAs(zip, '我的素材.zip');
}
