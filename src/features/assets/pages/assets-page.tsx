...
import { useCopyText } from '@/shared/ui/use-copy-text';
import { formatBytes, readFileAsDataUrl } from '@/shared/media/image-utils';
...
import type { Asset, AssetKind, ImageAsset } from '../domain/asset';
import { useAssetStore } from '../stores/use-asset-store';
import { exportAssets, readAssetPackage } from '../domain/asset-transfer';
...
export default function AssetsPage() {
  ...
  const validAssets = useMemo(
    () =>
      assets.filter(
        (asset) => asset.kind === 'text' || asset.kind === 'image' || asset.kind === 'video',
      ),
    [assets],
  );
  ...
  const saveAsset = async () => {
    ...
    const base = {
      ...
      coverUrl:
        ...
        (values.kind === 'image' && imageDraft ? imageDraft.dataUrl : ''),
      tags: values.tags || [],
  ...
  const downloadImage = (asset: Asset) => {
    ...
    saveAs(
      asset.kind === 'video' ? asset.data.url : asset.data.dataUrl,
  ...
  return (
    <CatalogPageShell gridTone="soft">
      ...
      <input
        ...
        className="hidden"
        onChange={(event) => void importAssetZip(event.target.files?.[0])}
      />
        ...
        <>
          <div className="flex items-start justify-between gap-3">
            ...
            <Tag className="m-0 shrink-0 text-[11px]">
              {asset.kind === 'image' ? '图片' : asset.kind === 'video' ? '视频' : '文本'}
            </Tag>
          </div>
          ...
          <div className="rounded-lg border border-border p-4">
            ...
            {asset.kind === 'text' ? (
              ...
              <Typography.Text className="mt-2 block">
                {asset.data.width}x{asset.data.height} · {formatBytes(asset.data.bytes)} ·{' '}
                {asset.data.mimeType}
              </Typography.Text>
            )}
          </div>
