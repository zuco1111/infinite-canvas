...
import { publicAssetPath } from '@/shared/platform/public-assets';
import { DiaTextReveal } from '@/shared/ui/dia-text-reveal';
import { Select, SelectContent, SelectItem, SelectTrigger } from '@/shared/ui/select';
import { CanvasPromptLibrary } from '@/features/prompts';
import {
  AgentChatComposer,
  AgentChatMessage,
  AgentModeSwitch,
...
function generationToolDefinition(
  name: string,
  description: string,
...
const ONLINE_AGENT_TOOLS: ResponseFunctionTool[] = [
  ...
  toolDefinition('canvas_create_text_node', '在当前画布创建单个文本节点。', {
    text: { type: 'string' },
    x: { type: 'number' },
    y: { type: 'number' },
    title: { type: 'string' },
  ...
  toolDefinition(
    'canvas_update_node',
    '更新节点基础字段或 metadata。',
    { id: { type: 'string' }, patch: JSON_RECORD_SCHEMA, metadata: JSON_RECORD_SCHEMA },
    ['id'],
  ),
  toolDefinition(
  ...
  toolDefinition('canvas_set_viewport', '调整画布视口。', { viewport: VIEWPORT_SCHEMA }, [
    'viewport',
  ]),
  toolDefinition(
    'canvas_run_generation',
    '触发指定节点生成，通常用于配置节点或文本/图片/视频/音频节点。',
    { nodeId: { type: 'string' }, mode: GENERATION_MODE_SCHEMA, prompt: { type: 'string' } },
    ['nodeId'],
  ),
];
...
type OnlineToolResult =
  { ok: true; message: string; data?: unknown } | { ok: false; message: string };
type OnlineExecutedToolCall = { toolCallId: string; name: string; result: OnlineToolResult };
type PendingOnlineToolContext = {
...
export function CanvasAssistantPanel({
  ...
  const cleanupImages = useCleanupAssetResources();
  const isAiConfigReady = useIsAiConfigReady();
  const openConfigDialog = useOpenConfigDialog();
  const updateConfig = useUpdateAiConfig();
  const confirmTools = useLocalAgentConfirmTools();
  const setAgentState = useSetLocalAgentState();
  ...
  const [localSessions, setLocalSessions] = useState<CanvasAssistantSession[]>(() =>
  ...
  const activeSession = useMemo(
    () =>
      ...
      safeSessions[0] ||
      null,
    [localActiveSessionId, safeSessions],
  );
  ...
  const selectedReferences = useMemo(
    () => allSelectedReferences.filter((item) => !removedReferenceIds.has(item.id)),
    [allSelectedReferences, removedReferenceIds],
  );
  ...
  const updateSession = (
    sessionId: string,
    updater: (session: CanvasAssistantSession) => CanvasAssistantSession,
  ) => {
  ...
  const addOnlineLog = (title: string, data?: unknown) =>
    setOnlineLogs((prev) =>
      [{ id: nanoid(), time: new Date().toLocaleTimeString(), title, data }, ...prev].slice(0, 80),
    );
  ...
  const removeSessions = (ids: string[]) => {
    ...
    if (!next.length) {
      ...
      setLocalActiveSessionId(
        localActiveSessionId && ids.includes(localActiveSessionId)
          ? next[0].id
  ...
  const sendMessage = async (
    ...
    savedReferences?: CanvasAssistantReference[],
  ) => {
    const requestConfig = {
      ...effectiveConfig,
    ...
    const userMessage: CanvasAssistantMessage = {
      id: nanoid(),
      role: 'user',
      text,
      references: refs,
    ...
    addOnlineLog('发送请求', {
      ...
      connectionCount: snapshotRef.current.connections.length,
    });
    setPrompt('');
    setIsRunning(true);
      ...
      if (result.toolCalls.length) {
        ...
        if (confirmTools && writableCalls.length) {
          upsertMessage(sessionId, {
            id: assistantId,
            role: 'assistant',
            text: result.content || streamed || '准备执行工具，等待确认。',
          });
          const toolMessageId = nanoid();
          pendingToolContextRef.current.set(toolMessageId, {
            messages,
          ...
          const toolMessage: CanvasAssistantMessage = {
            ...
            detail: { status: 'pending', step: loop.step, toolCalls: result.toolCalls },
          };
        ...
        upsertMessage(sessionId, {
          id: assistantId,
          role: 'assistant',
          text: result.content || streamed || '没有返回内容。',
        });
  ...
  const continueOnlineToolLoop = async (
    sessionId: string,
    assistantId: string,
    messages: ResponseInputMessage[],
    result: { content: string; toolCalls: ResponseToolCall[] },
    step: number,
  ) => {
    ...
    await continueOnlineToolLoopAfterResults(
      sessionId,
      assistantId,
      messages,
      result.toolCalls,
  ...
  const continueOnlineToolLoopAfterResults = async (
    sessionId: string,
    assistantId: string,
    messages: ResponseInputMessage[],
    toolCalls: ResponseToolCall[],
    toolResults: OnlineExecutedToolCall[],
    step: number,
  ) => {
    ...
    if (next.toolCalls.length) {
      ...
      if (confirmTools && writableCalls.length) {
        upsertMessage(sessionId, {
          id: assistantId,
          role: 'assistant',
          text: next.content || streamed || '准备执行工具，等待确认。',
        });
        const toolMessageId = nanoid();
        pendingToolContextRef.current.set(toolMessageId, {
        ...
        appendMessage(sessionId, {
          ...
          detail: { status: 'pending', step: step + 1, toolCalls: next.toolCalls },
        });
  ...
  const executeOps = (ops: CanvasAgentOp[]) => {
    ...
    return {
      ...
      after: JSON.parse(snapshotSignature(next)),
    };
  };
  ...
  const executeOnlineTool = (name: string, args: Record<string, unknown>): OnlineToolResult => {
    ...
    try {
      if (name === 'canvas_get_state')
        return {
          ok: true,
          message: describeCanvasSnapshot(current),
          data: compactSnapshot(current),
        };
      ...
      if (name === 'canvas_get_selection') {
        ...
        return {
          ok: true,
          message: `当前选中 ${ids.size} 个节点。`,
          data: {
            nodes: compactSnapshot({
              ...current,
      ...
      return {
        ...
        message: result.changed
          ? summarizeCanvasAgentOps(ops) || '画布操作已执行。'
          : result.noopReason,
        data: result,
      };
    } catch (error) {
  ...
  const executeOnlineToolCall = (toolCall: ResponseToolCall): OnlineExecutedToolCall => {
    try {
      const result = executeOnlineTool(
      ...
      return {
        ...
        result: { ok: false, message: error instanceof Error ? error.message : '工具参数错误' },
      };
    }
  };
    ...
    toolCalls.forEach((toolCall) => {
      if (stopped) {
        results.push({
          toolCallId: toolCall.id,
          name: toolCall.function.name,
          result: { ok: false, message: '前一个工具调用失败，未继续执行。' },
        });
        return;
      }
  ...
  const approveOnlineTool = async (messageId: string) => {
    ...
    const session = safeSessions.find((session) =>
      session.messages.some((item) => item.id === messageId),
    );
    ...
    try {
      ...
      await continueOnlineToolLoopAfterResults(
        ...
        toolCalls,
        results,
        pendingContext?.step || Number(detail.step) || 1,
      );
    } catch (error) {
  ...
  const rejectOnlineTool = (messageId: string) => {
    ...
    if (session)
      upsertMessage(session.id, {
        ...
        text: '工具调用已取消',
        detail: {
          ...objectDetail(session.messages.find((item) => item.id === messageId)?.detail),
          status: 'rejected',
        },
      });
  };
  ...
  const startResize = () => {
    const move = (event: MouseEvent) =>
      setWidth(Math.min(760, Math.max(320, window.innerWidth - event.clientX)));
    const stop = () => {
      setResizing(false);
        ...
        <div className="thin-scrollbar min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4">
          {view === 'history' ? (
            ...
            <div className="flex h-full flex-col items-center justify-center px-1 text-center">
              <div
                className="relative font-serif text-4xl font-bold italic tracking-normal"
                ...
                <AssistantReferenceChip
                  ...
                  onRemove={() => {
                    ...
                    if (selectedNodeIds.has(item.id))
                      onSelectNodeIds(
                        new Set(Array.from(selectedNodeIds).filter((nodeId) => nodeId !== item.id)),
                      );
                  }}
                />
              ))}
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            >
              <Switch
                ...
                onChange={(confirmTools) => setAgentState({ confirmTools })}
              />
              工具确认
            </label>
            <Tooltip title="收起对话">
...
function AgentTextModelPicker({
  ...
  const options = useMemo(
    () =>
      Array.from(new Set([value, ...selectableModelsByCapability(config, 'text')].filter(Boolean))),
    [config, value],
  );
  const current = value || '';
  return (
    <Select value={current} onValueChange={onChange}>
      <SelectTrigger
        ...
        title={
          current
            ? `${modelOptionName(current)} · ${resolveModelChannel(config, current).name}`
            : '选择文本模型'
        }
      ...
      >
        ...
        <span className="min-w-0 truncate">
          {current ? modelOptionName(current) : '选择文本模型'}
        </span>
        {current ? (
          <span className="shrink-0 opacity-55">{resolveModelChannel(config, current).name}</span>
        ) : null}
      </SelectTrigger>
      <SelectContent
        data-canvas-no-zoom
        ...
        {options.length ? (
          options.map((model) => (
            <SelectItem
              key={model}
              value={model}
              textValue={`${modelOptionName(model)} ${resolveModelChannel(config, model).name}`}
            >
              <span className="flex min-w-0 items-center gap-2">
                ...
                <span className="shrink-0 text-xs opacity-55">
                  {resolveModelChannel(config, model).name}
                </span>
              </span>
            </SelectItem>
          ))
    ...
    <div className="space-y-3">
      ...
      {sessions.map((session) => (
        <div
          ...
          style={{
            borderColor: session.id === activeSession?.id ? theme.node.text : theme.node.stroke,
...
function OnlineAgentLogView({
  ...
  logs: OnlineAgentLog[];
  theme: (typeof canvasThemes)[keyof typeof canvasThemes];
  context: OnlineAgentLogContext;
  onClear: () => void;
}) {
  const [mode, setMode] = useState<'text' | 'json'>('text');
  const textareaRef = useRef<HTMLTextAreaElement>(null);
  const content =
    mode === 'text' ? formatOnlineLogText(logs, context) : formatOnlineLogJson(logs, context);
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            size="small"
            disabled={!lastError}
            onClick={() => lastError && void copy(formatOnlineLogText([lastError], context))}
          >
            最近错误
          </Button>
          <Button
...
function MessageReferences({ message }: { message: CanvasAssistantMessage }) {
  return (
    <div
      className={`flex max-w-[88%] flex-wrap gap-2 ${message.role === 'user' ? 'ml-auto justify-end' : 'ml-11 justify-start'}`}
    >
      {message.references?.map((item, index, references) => (
        <AssistantReferenceChip
          key={item.id}
          item={item}
          label={assistantImageReferenceLabel(references, index)}
        />
      ))}
    </div>
  );
...
function AssistantReferenceChip({
  item,
  label,
  onRemove,
}: {
  item: CanvasAssistantReference;
  label?: string;
  onRemove?: () => void;
}) {
  const theme = canvasThemes[useAppTheme()];
  ...
  return (
    <div
      className="group/chip relative inline-flex h-8 max-w-[150px] shrink-0 items-center gap-1.5 rounded-lg text-sm"
      style={{ color: theme.node.text }}
    >
      {item.dataUrl ? (
        <span className="relative block size-8 shrink-0">
          ...
          {label ? (
            <span className="absolute left-0.5 top-0.5 rounded bg-black/60 px-1 py-0.5 text-[8px] font-medium leading-none text-white">
              {label}
            </span>
          ) : null}
        </span>
        ...
        <span
          className="grid size-8 place-items-center rounded-lg border text-sm font-medium"
          style={{ background: theme.node.panel, borderColor: theme.node.activeStroke }}
        >
          {text}
        </span>
      )}
...
function formatOnlineLogText(logs: OnlineAgentLog[], context: OnlineAgentLogContext) {
  ...
  const body = logs
    .map((log, index) =>
      [
        `#${index + 1} ${log.time} ${log.title}`,
        log.data === undefined ? '' : stringifyLog(log.data),
      ]
        .filter(Boolean)
        .join('\n'),
    )
...
function formatOnlineLogJson(logs: OnlineAgentLog[], context: OnlineAgentLogContext) {
  return JSON.stringify(
    { context, logs: logs.map(({ time, title, data }) => ({ time, title, data })) },
    null,
    2,
  );
}
...
function onlineToolToOps(
  ...
  if (name === 'canvas_create_node') {
    ...
    if (nodeType === CanvasNodeType.Config)
      return [
        configNodeOp(
          stringOptional(input.id) || `config-${nanoid()}`,
          { ...recordOptional(input.metadata), ...input },
          x,
          y,
          config,
        ),
    ...
    return [
      {
        ...
        metadata: recordOptional(input.metadata) as CanvasNodeData['metadata'],
      },
    ];
  }
  if (name === 'canvas_create_text_node')
    return [textNodeOp(input, numberOr(input.x, nextCanvasX(snapshot)), numberOr(input.y, 0))];
    ...
    return items.map((item, index) =>
      textNodeOp(
        ...
        numberOr(
          item.x,
          direction === 'row'
            ? x + index * (NODE_DEFAULT_SIZE[CanvasNodeType.Text].width + gap)
            : x,
        ),
        numberOr(
          item.y,
          direction === 'row'
            ? y
            : y + index * (NODE_DEFAULT_SIZE[CanvasNodeType.Text].height + gap),
        ),
      ),
    );
  }
  if (name === 'canvas_create_image_prompt_flow')
    return generationFlowOps({ ...input, mode: 'image' }, snapshot, config);
  if (name === 'canvas_create_config_node') {
    ...
    return [
      configNodeOp(
        configId,
        input,
        numberOr(input.x, nextCanvasX(snapshot)),
        numberOr(input.y, 0),
        config,
      ),
      ...(input.autoRun ? [runGenerationOp(configId, mode, stringOptional(input.prompt))] : []),
    ];
  }
  if (name === 'canvas_create_generation_flow') return generationFlowOps(input, snapshot, config);
  if (name === 'canvas_generate_text')
    return generationFlowOps({ ...input, mode: 'text', autoRun: true }, snapshot, config);
  if (name === 'canvas_generate_image')
    return generationFlowOps({ ...input, mode: 'image', autoRun: true }, snapshot, config);
  if (name === 'canvas_generate_video')
    return generationFlowOps({ ...input, mode: 'video', autoRun: true }, snapshot, config);
  if (name === 'canvas_generate_audio')
    return generationFlowOps({ ...input, mode: 'audio', autoRun: true }, snapshot, config);
  if (name === 'canvas_update_node')
    return [
      {
        type: 'update_node',
        id: requireString(input.id, 'id'),
        patch: recordOptional(input.patch) as Partial<CanvasNodeData> | undefined,
        metadata: recordOptional(input.metadata) as CanvasNodeData['metadata'],
      },
    ];
    ...
    return [
      {
        type: 'update_node',
        id: requireString(input.id, 'id'),
        patch: stringOptional(input.title) ? { title: stringOptional(input.title) } : undefined,
        metadata: { content: requireString(input.text, 'text'), status: 'success' },
      },
    ];
  if (name === 'canvas_move_nodes') {
    return requireRecordArray(input.items, 'items').map((item) => {
      ...
      return {
        ...
        patch: {
          position: {
            x: numberOr(item.x, (current?.position.x || 0) + numberOr(item.dx, 0)),
            y: numberOr(item.y, (current?.position.y || 0) + numberOr(item.dy, 0)),
          },
        },
      };
    });
    ...
    return [
      {
        ...
        metadata:
          typeof input.freeResize === 'boolean' ? { freeResize: input.freeResize } : undefined,
      },
    ];
  if (name === 'canvas_delete_nodes')
    return [{ type: 'delete_node', ids: requireStringArray(input.ids, 'ids') }];
  ...
  if (name === 'canvas_select_nodes')
    return [{ type: 'select_nodes', ids: requireStringArray(input.ids, 'ids') }];
  if (name === 'canvas_set_viewport')
    return [{ type: 'set_viewport', viewport: requireViewport(input.viewport) }];
  if (name === 'canvas_run_generation')
    return [
      runGenerationOp(
        requireString(input.nodeId, 'nodeId'),
        generationMode(input.mode),
        stringOptional(input.prompt),
      ),
    ];
...
function generationFlowOps(
  ...
  return [
    ...
    configNodeOp(
      configId,
      { ...input, prompt: tokens.join('\n') },
      x + NODE_DEFAULT_SIZE[CanvasNodeType.Text].width + 80,
      y,
      config,
    ),
...
function configNodeOp(
  ...
  return {
    ...
    metadata: cleanRecord({
      ...
      count:
        numberOptional(input.count) ??
        generationCount(mode === 'image' ? config.canvasImageCount || config.count : config.count),
...
function toolCallsFromDetail(detail: Record<string, unknown>): ResponseToolCall[] {
  return Array.isArray(detail.toolCalls)
    ? (detail.toolCalls.filter(isResponseToolCall) as ResponseToolCall[])
    : [];
}
...
function requireStringArray(value: unknown, field: string): string[] {
  ...
  if (!value.every((item) => typeof item === 'string' && Boolean(item)))
    throw new Error(`${field} 必须只包含非空字符串`);
...
function toCanvasAgentOp(value: unknown): CanvasAgentOp {
  ...
  if (type === 'add_node') {
    return {
      ...
      position: recordOptional(item.position)
        ? {
            x: requireNumber(objectDetail(item.position).x, 'position.x'),
            y: requireNumber(objectDetail(item.position).y, 'position.y'),
          }
        : undefined,
  ...
  if (type === 'update_node')
    return {
      ...
      patch: recordOptional(item.patch) as Partial<CanvasNodeData> | undefined,
      metadata: recordOptional(item.metadata) as CanvasNodeData['metadata'],
    };
  if (type === 'delete_node')
    return {
      type,
      id: stringOptional(item.id),
      ids: Array.isArray(item.ids) ? requireStringArray(item.ids, 'ids') : undefined,
    };
  if (type === 'delete_connections')
    return {
      ...
      id: stringOptional(item.id),
      ids: Array.isArray(item.ids) ? requireStringArray(item.ids, 'ids') : undefined,
      all: typeof item.all === 'boolean' ? item.all : undefined,
    };
  if (type === 'connect_nodes')
    return {
      type,
      id: stringOptional(item.id),
      fromNodeId: requireString(item.fromNodeId, 'fromNodeId'),
      toNodeId: requireString(item.toNodeId, 'toNodeId'),
    };
...
function requireNodeType(value: unknown): CanvasNodeType {
  if (Object.values(CanvasNodeType).includes(value as CanvasNodeType))
...
function requireViewport(value: unknown) {
  const item = objectDetail(value);
  return {
    x: requireNumber(item.x, 'viewport.x'),
    y: requireNumber(item.y, 'viewport.y'),
    k: requireNumber(item.k, 'viewport.k'),
  };
}
...
function nextCanvasX(snapshot: CanvasAgentSnapshot) {
  return snapshot.nodes.length
    ? Math.max(...snapshot.nodes.map((node) => node.position.x + node.width)) + 80
    : 0;
}
...
function resolveGenerationModel(
  ...
  return normalized && selectableModelsByCapability(config, mode).includes(normalized)
    ? normalized
    : defaultGenerationModel(config, mode);
}
...
function explainNoop(ops: CanvasAgentOp[], snapshot: CanvasAgentSnapshot) {
  ...
  const deleteConnectionOps = ops.filter(
    (op): op is Extract<CanvasAgentOp, { type: 'delete_connections' }> =>
      op.type === 'delete_connections',
  );
  const connectOps = ops.filter(
    (op): op is Extract<CanvasAgentOp, { type: 'connect_nodes' }> => op.type === 'connect_nodes',
  );
  const deleteNodeOps = ops.filter(
    (op): op is Extract<CanvasAgentOp, { type: 'delete_node' }> => op.type === 'delete_node',
  );
  const updateOps = ops.filter(
    (op): op is Extract<CanvasAgentOp, { type: 'update_node' }> => op.type === 'update_node',
  );
  const selectOps = ops.filter(
    (op): op is Extract<CanvasAgentOp, { type: 'select_nodes' }> => op.type === 'select_nodes',
  );
  const generationOps = ops.filter(
    (op): op is Extract<CanvasAgentOp, { type: 'run_generation' }> => op.type === 'run_generation',
  );
  if (deleteConnectionOps.length && !snapshot.connections.length) return '画布当前没有连线可删除。';
  if (
    deleteConnectionOps.length &&
    deleteConnectionOps.every(
      (op) =>
        !op.all &&
        [...(op.ids || []), ...(op.id ? [op.id] : [])].every((id) => !connectionIds.has(id)),
    )
  )
    return '没有找到要删除的连线。';
  if (
    ...
    return '这些节点已经存在对应连线，无需重复连接。';
  if (
    connectOps.length &&
    connectOps.every((op) => !nodeIds.has(op.fromNodeId) || !nodeIds.has(op.toNodeId))
  )
    return '没有找到要连接的节点。';
  if (
    deleteNodeOps.length &&
    deleteNodeOps.every((op) => op.nodeType === CanvasNodeType.Config) &&
    !snapshot.nodes.some((node) => node.type === CanvasNodeType.Config)
  )
    return '画布当前没有生成配置节点可删除。';
  if (
    deleteNodeOps.length &&
    deleteNodeOps.every((op) =>
      [...(op.ids || []), ...(op.id ? [op.id] : [])].every((id) => !nodeIds.has(id)),
    )
  )
    return '没有找到要删除的节点。';
  if (updateOps.length && updateOps.every((op) => !nodeIds.has(op.id)))
    return '没有找到要更新的节点。';
  if (selectOps.length && selectOps.every((op) => !(op.ids || []).some((id) => nodeIds.has(id))))
    return '没有找到要选择的节点。';
  if (generationOps.length && generationOps.every((op) => !nodeIds.has(op.nodeId)))
    return '没有找到要触发生成的节点。';
  if (ops.every((op) => op.type === 'set_viewport')) return '视图已经是目标状态。';
  if (
    selectOps.length &&
    selectOps.every(
      (op) => JSON.stringify(op.ids || []) === JSON.stringify(snapshot.selectedNodeIds),
    )
  )
    return '选区已经是目标状态。';
  ...
  return [
      ...
      })),
    {
      ...
      content: [
        ...refs.flatMap((item) =>
          item.text
            ? [{ type: 'text' as const, text: `选中节点 ${item.title}：${item.text}` }]
            : [],
        ),
        {
          type: 'text',
          text: `当前画布：${JSON.stringify(compactSnapshot(snapshot))}\n\n用户需求：${userMessage.text}`,
        },
        ...(await Promise.all(
          refs
            ...
            .map(async (item) => ({
              ...
              image_url: { url: await imageToDataUrl(item) },
            })),
        )),
      ],
    },
