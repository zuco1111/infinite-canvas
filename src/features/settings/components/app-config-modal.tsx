...
import { fetchChannelModels } from '@/features/generation';
import {
  syncAppDataToWebdav,
  type AppSyncDomainKey,
  type AppSyncProgressEvent,
} from '@/features/sync';
import { testWebdavConnection, WEBDAV_MANIFEST_FILE_NAME } from '@/features/sync';
...
export function AppConfigModal() {
  ...
  const finishConfig = () => {
    const ready = config.channels.some(
      (channel) => channel.baseUrl.trim() && channel.apiKey.trim() && channel.models.length,
    );
  ...
  const updateChannelApiFormat = (channel: ModelChannel, apiFormat: ApiCallFormat) => {
    const baseUrl =
      ...
      channel.baseUrl.trim() === defaultBaseUrlForApiFormat(channel.apiFormat)
  ...
  const refreshChannelModels = async (channel: ModelChannel) => {
    ...
    try {
      ...
      updateChannels(
        config.channels.map((item) => (item.id === channel.id ? { ...item, models } : item)),
      );
  ...
  const refreshAllModels = async () => {
    const runnable = config.channels.filter(
      (channel) => channel.baseUrl.trim() && channel.apiKey.trim(),
    );
    if (!runnable.length) {
    ...
    try {
      const entries = await Promise.all(
        runnable.map(async (channel) => [channel.id, await fetchChannelModels(channel)] as const),
      );
      const modelMap = new Map(entries);
      updateChannels(
        config.channels.map((channel) =>
          modelMap.has(channel.id)
            ? { ...channel, models: modelMap.get(channel.id) || [] }
            : channel,
        ),
      );
  ...
  const updateCapabilityModels = (group: ModelGroup, models: string[]) => {
    const next = uniqueModels(
      models.map((model) => normalizeModelOptionValue(model, config.channels)).filter(Boolean),
    );
  ...
  const syncWebdav = async () => {
    ...
    try {
      ...
      message.success(
        `同步完成：${result.projects} 个画布，${result.assets} 个素材，${result.imageLogs + result.videoLogs} 条记录，本次上传 ${result.uploadedFiles} 个文件 ${formatBytes(result.uploadedBytes)}`,
      );
    } catch (error) {
                ...
                <AppSurface className="mb-4 flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex shrink-0 gap-2">
                    <Button
                      ...
                      loading={Boolean(loadingChannelId)}
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        ...
                        <Form.Item label="调用格式" className="mb-0">
                          <Select
                            ...
                            onChange={(value: ApiCallFormat) =>
                              updateChannelApiFormat(channel, value)
                            }
                          />
                        </Form.Item>
                        ...
                        <Form.Item label="API Key" className="mb-0">
                          <Input.Password
                            ...
                            onChange={(event) =>
                              updateChannel(channel.id, { apiKey: event.target.value })
                            }
                          />
                        </Form.Item>
            ...
            children: (
              <Form layout="vertical" requiredMark={false}>
                <div className="grid gap-4 md:grid-cols-4">
                  <Form.Item
                    label="画布默认生图张数"
                    extra="新建画布生图和配置节点默认使用，单个节点仍可单独覆盖。"
                    className="mb-4"
                  >
                    <Input
                      type="number"
                      ...
                      onBlur={(event) =>
                        updateConfig('canvasImageCount', normalizeImageCount(event.target.value))
                      }
                    />
                  </Form.Item>
                  ...
                  <Form.Item label="默认音频语速" className="mb-4">
                    <Input
                      ...
                      onBlur={(event) =>
                        updateConfig('audioSpeed', normalizeAudioSpeedValue(event.target.value))
                      }
                    />
                  </Form.Item>
                </div>
                <Form.Item label="默认音频指令" className="mb-4">
                  <Input.TextArea
                    ...
                    onChange={(event) => updateConfig('audioInstructions', event.target.value)}
                  />
                </Form.Item>
              ...
              <Form layout="vertical" requiredMark={false}>
                <AppSurface as="section">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <Form.Item
                      label="远程目录"
                      extra={`会在该目录下分业务目录保存，每个目录包含 ${WEBDAV_MANIFEST_FILE_NAME} 和 files/`}
                      className="mb-4"
                    >
                      <Input
                        value={webdav.directory}
                        placeholder="infinite-canvas"
                        onChange={(event) => updateWebdavConfig('directory', event.target.value)}
                      />
                    </Form.Item>
...
function withChannels(config: AiConfig, channels: ModelChannel[]): AiConfig {
  ...
  const imageModels = keepOrSuggest(
    config.imageModels,
    filterModelsByCapability(models, 'image'),
    models,
  );
  const videoModels = keepOrSuggest(
    config.videoModels,
    filterModelsByCapability(models, 'video'),
    models,
  );
  const textModels = keepOrSuggest(
    config.textModels,
    filterModelsByCapability(models, 'text'),
    models,
  );
  const audioModels = keepOrSuggest(
    config.audioModels,
