...
export const useConfigStore = create<ConfigStore>()((set, get) => ({
  ...
  updateConfig: (key, value) => {
    ...
    set({ config });
    persistConfigSettings(config, get().webdav);
  },
  updateWebdavConfig: (key, value) => {
    ...
    set({ webdav });
    persistConfigSettings(get().config, webdav);
  },
...
function hydrateConfigSettings() {
  ...
  settingsHydrationPromise = settingsRepository.read().then((settings) => {
    if (settingsDirty) {
      ...
      persistConfigSettings(state.config, state.webdav);
      useConfigStore.setState({ hydrated: true });
      return;
    }
    useConfigStore.setState({
      ...normalizePersistedSettings(settings.config, settings.webdav),
      hydrated: true,
    });
  });
...
function persistConfigSettings(config: AiConfig, webdav: WebdavSyncConfig) {
  void settingsRepository.update({ config, webdav });
}

function normalizePersistedSettings(
  configValue: unknown,
  webdavValue: unknown,
): Pick<ConfigStore, 'config' | 'webdav'> {
  const persistedConfig = (configValue || {}) as Partial<AiConfig>;
  const persistedWebdav = (webdavValue || {}) as Partial<WebdavSyncConfig>;
...
function normalizeModelList(models: string[], channels: ModelChannel[]) {
  ...
  return Array.from(new Set((models || []).map((model) => model.trim()).filter(Boolean)))
    ...
    .filter(
      (model) =>
        !allModelOptions.length || allModelOptions.includes(model) || !isChannelModelValue(model),
    );
}
...
function uniqueRawModels(models: string[]) {
  return Array.from(
    new Set((models || []).map((model) => modelOptionName(model).trim()).filter(Boolean)),
  );
}
...
export function buildApiUrl(baseUrl: string, path: string) {
  ...
  return maybeProxyRemoteUrl(apiUrl, 'ai');
}
