...
import { getMediaBlob, resolveMediaUrl, setMediaBlob } from '@/shared/storage/file-storage';
import { getImageBlob, resolveImageUrl, setImageBlob } from '@/shared/storage/image-storage';
import { downloadWebdavFile, uploadWebdavFile, WEBDAV_MANIFEST_FILE_NAME } from './webdav-sync';
import { assetRepository, type Asset } from '@/features/assets';
import type { WebdavSyncConfig } from '@/features/settings';
import { canvasProjectRepository, type CanvasProject } from '@/features/canvas';
import { imageGenerationLogRepository, videoGenerationLogRepository } from '@/features/generation';
...
export async function syncAppDataToWebdav(
  ...
  const [canvas, assets, imageLogs, videoLogs] = await Promise.all([
    syncDomain<CanvasDomainData>(config, onProgress, {
      ...
      label: '画布',
      emptyData: { projects: [] },
      localData: async () => ({ projects: await canvasProjectRepository.list() }),
      mergeData: (local, remote) => ({
        projects: mergeById(local.projects, remote.projects, 'updatedAt'),
      }),
      applyData: async (data) => canvasProjectRepository.replace(data.projects),
    }),
    syncDomain<AssetDomainData>(config, onProgress, {
      key: 'assets',
      label: '我的素材',
      emptyData: { assets: [] },
      localData: async () => ({ assets: await assetRepository.list() }),
      mergeData: (local, remote) => ({
        assets: mergeById(local.assets, remote.assets, 'updatedAt'),
      }),
      applyData: async (data) =>
        assetRepository.replace(await Promise.all(data.assets.map(hydrateAsset))),
    }),
    syncDomain<LogDomainData>(config, onProgress, {
      key: 'image-workbench',
      label: '生图工作台',
      emptyData: { logs: [] },
      localData: async () => ({ logs: await imageGenerationLogRepository.list<StoredLog>() }),
      mergeData: (local, remote) => ({ logs: mergeById(local.logs, remote.logs, 'createdAt') }),
      applyData: async (data) => imageGenerationLogRepository.replace(data.logs),
    }),
    syncDomain<LogDomainData>(config, onProgress, {
      key: 'video-workbench',
      label: '视频创作台',
      emptyData: { logs: [] },
      localData: async () => ({ logs: await videoGenerationLogRepository.list<StoredLog>() }),
      mergeData: (local, remote) => ({ logs: mergeById(local.logs, remote.logs, 'createdAt') }),
      applyData: async (data) => videoGenerationLogRepository.replace(data.logs),
    }),
  ]);
