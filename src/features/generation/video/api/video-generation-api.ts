...
import { getMediaBlob, uploadMediaFile, type UploadedFile } from '@/shared/storage/file-storage';
...
export async function requestVideoGeneration(
  ...
  >({
    poll: () => pollVideoGenerationTask(config, task, options),
    isCompleted: (state): state is Extract<VideoGenerationTaskState, { status: 'completed' }> =>
      state.status === 'completed',
    isFailed: (state) => state.status === 'failed',
    getFailureMessage: (state) => (state.status === 'failed' ? state.error : '视频生成失败'),
