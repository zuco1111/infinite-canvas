...
const categories: PromptCategory[] = [
  ...
  },
  {
    ...
    build: () => buildYouMindPrompts(youMindGptImage2RawBase, 'youmind-gpt-image-2', 'gpt-image-2'),
  },
  {
...
async function buildGptImage2Prompts() {
  const data =
    (
      ...
      }>(gptImage2RawBase, 'data/ingested_tweets.json')
    ).records || [];
  const cases = new Map<string, string>();
  const markdowns = await Promise.all(
    gptImage2CaseFiles.map((file) => fetchText(gptImage2RawBase, file)),
  );
...
async function buildAwesomeGptImagePrompts() {
  ...
  for (const section of splitBeforeHeading(markdown, '## ')) {
    ...
    for (const block of splitBeforeHeading(section, '### ')) {
      ...
      items.push(
        defaultPrompt(
          `awesome-gpt-image-${leftPad(items.length + 1)}`,
          title,
          prompt,
          images[0] || '',
          tags,
          markdownPreview(images),
        ),
      );
    }
  }
...
async function buildAwesomeGpt4oImagePrompts() {
  ...
  for (const block of splitBeforeHeading(markdown, '### ')) {
    ...
    items.push(
      defaultPrompt(
        `awesome-gpt4o-image-prompts-${leftPad(items.length + 1)}`,
        title,
        prompt,
        images[0] || '',
        ['gpt4o'],
...
async function buildYouMindPrompts(baseUrl: string, idPrefix: string, modelTag: string) {
  ...
  for (const block of splitBeforeHeading(markdown, '### ')) {
    ...
    items.push(
      defaultPrompt(
        `${idPrefix}-${leftPad(items.length + 1)}`,
        title,
        prompt,
        images[0] || '',
        youMindTags(title, modelTag),
        markdownPreview(images),
      ),
    );
  }
...
async function buildDavidWuGptImage2Prompts() {
  ...
  >(davidWuGptImage2RawBase, 'prompts.json');
  return data
    .map((item, index) => {
      ...
      const preview = [item.title_en, item.note, image ? `![](${image})` : '']
        .filter(Boolean)
        .join('\n\n');
      return defaultPrompt(
        `davidwu-gpt-image2-prompts-${leftPad(item.id || index + 1)}`,
        title,
        prompt,
        image,
        davidWuTags(item),
...
function defaultPrompt(
  ...
  tags: string[],
  preview: string,
): Omit<Prompt, 'category' | 'githubUrl'> {
