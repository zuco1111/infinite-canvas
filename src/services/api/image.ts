...
function parseToolResponse(payload: ResponseApiPayload): ToolResponseResult {
  ...
  const toolCalls = output
    .filter(
      (item): item is Extract<ResponseApiOutputItem, { type?: 'function_call' }> =>
...
function responseErrorMessage(value: unknown) {
  ...
  return (
    stringValue(value.msg) || stringValue(error?.message) || stringValue(responseError?.message)
  );
}
...
function toGeminiBody(
  ...
  const systemText = [
    ...
    ...messages.flatMap((message) =>
      !('type' in message) && message.role === 'system' ? [geminiTextContent(message.content)] : [],
    ),
  ]
    .filter(Boolean)
    .join('\n\n');
  const contents = toGeminiContents(
    messages.filter((message) => ('type' in message ? true : message.role !== 'system')),
  );
  return {
    contents,
...
function toGeminiParts(content: ResponseMessageContent): GeminiPart[] {
  ...
  return content.map((item) =>
    item.type === 'text' ? { text: item.text } : toGeminiImagePart(item.image_url.url),
  );
}
  ...
  try {
    if (requestConfig.apiFormat === 'gemini') {
      return await requestGeminiStreamingResponse(
        requestConfig,
        toGeminiBody(requestConfig, messages, toGeminiToolOptions(tools, toolChoice)),
        onDelta,
        options,
      );
    }
