...
export class CanvasSession {
  ...
  updateState(body: unknown, clientId?: string) {
    this.canvasState = {
      ...((body && typeof body === 'object' && !Array.isArray(body) ? body : {}) as Record<
        string,
        unknown
      >),
      clientId,
  ...
  async callTool(name: unknown, rawInput: unknown) {
        ...
        ops: [
          {
            ...
            title: data.title,
            position: { x: data.x ?? nextCanvasX(this.canvasState), y: data.y ?? 0 },
            width: data.width,
            height: data.height,
    ...
    if (tool === 'canvas_update_node') {
      ...
      input = {
        ops: [{ type: 'update_node', id: data.id, patch: data.patch, metadata: data.metadata }],
      };
      tool = 'canvas_apply_ops';
    }
    ...
    if (tool === 'canvas_connect_nodes') {
      ...
      input = {
        ops: data.connections.map((connection) => ({ type: 'connect_nodes', ...connection })),
      };
      tool = 'canvas_apply_ops';
    }
    ...
    if (tool === 'canvas_set_viewport') {
      input = {
        ops: [{ type: 'set_viewport', viewport: (input as { viewport: unknown }).viewport }],
      };
      tool = 'canvas_apply_ops';
    }
  ...
  private async requestCanvasTool(name: ToolName, input: Record<string, unknown>) {
    ...
    const client =
      this.clients.get(this.canvasState?.clientId || '') || this.clients.values().next().value;
