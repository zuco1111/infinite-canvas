...
决策：新的设计系统与视觉优化是一项无法一次完成的长期项目，先将完整方案落到 `docs/DESIGN_SYSTEM_REFACTOR_PLAN.md`，再逐阶段推进。

决策：第一次正式盘点应同时覆盖基础组件、高级组件、领域组件、Token、第三方组件、原生控件、CSS/Tailwind 和内联样式；评审按层级进行，但不等基础层完成后才盘点高级层。

决策：在修改生产视觉前先建立当前态库存、当前视觉基线和 Design Lab；Design Lab 必须区分当前实现与候选目标。

决策：具体 Token、组件合并/拆分、变体、例外和视觉规格由用户逐项批准。本次对方案的认可不构成对所有生产视觉修改的一次性授权。

决策：Foundations/Primitives 获批后先在低风险真实页面试点；Patterns/高级组件获批后再在复杂工作台试点；最终按 feature 分批迁移，不进行一次性全站替换，Canvas 主要迁移后置。

决策：每个阶段和迁移批次必须维护库存、决策、迁移状态、验证结果和项目文档；未批准或无法确认的项目只阻塞对应切片，不得由代理猜测。

来源：用户于 2026-07-11 确认先完整文档化已提出的方案，后续再逐步实施。

决策：用户于 2026-07-12 明确批准开始设计系统阶段 3，并明确本切片先审计现有样式，把相似内容放在一起以便比较和判断。该授权只允许建立只读对比结构和审计证据，不构成任何目标 Token、组件合并、视觉值或生产迁移批准。
...
适用范围：该决策记录 Phase 5/6 等值抽取阶段的历史边界，不代表后续设计系统已经确定 Canonical Token Source 或 App/Canvas 的最终共享层级。新设计系统工作的当前规则与开放决策见 `docs/DESIGN_SYSTEM_REFACTOR_PLAN.md`。

### Phase 1–6 历史视觉一致性验证方法

决策：以 `docs/VISUAL_BASELINE.md` 的截图集作为迁移基线。迁移切片必须覆盖受影响页面或组件的桌面截图；移动关键路径和主题切换在涉及布局或主题时补充验证。

来源：Phase 1 运行时视觉基线。

适用范围：该基线是旧项目迁移与历史等值阶段的证据。新的视觉优化开始前必须为当前仓库重新采集改前基线；后续基线规则见 `docs/DESIGN_SYSTEM_REFACTOR_PLAN.md`。
...
Phase 8 已完成，当前没有阻塞其完成状态的开放架构决策。

### 设计系统与视觉优化

状态：部分开放。设计系统阶段决策的唯一详细清单、稳定 ID、状态、最晚关闭阶段和阻塞范围统一维护在 `docs/DESIGN_SYSTEM_REFACTOR_PLAN.md` 的“15.2 决策清单与最晚关闭阶段”。本文件不复制该清单，避免两处内容漂移。

当前规则：DS-O5 已由用户于 2026-07-11 确认；阶段 1 的库存覆盖、分类、风险、59 张当前基线和新旧双向追踪规则已由用户于 2026-07-11 确认。用户随后于 2026-07-11 按推荐方案确认 DS-O2、DS-O3、DS-O4、DS-O14，并批准阶段 2；阶段 2 当前态 Design Lab 已于同日通过用户验收并完成。用户于 2026-07-12 批准阶段 3 先执行现有样式对比审计；DS-O1、DS-O6、DS-O7、DS-O8、DS-O9、DS-O10、DS-O13、DS-O15 等决策仍保持开放，并继续阻塞目标 Token 体系和生产替换，但不阻塞只读审计。

已确认范围例外：用户于 2026-07-11 明确冻结 `/` 首页在 Light/Dark、Web 桌面/移动和 Electron 桌面下的非导航视觉；公共导航仍可按组件审批结果修改。该决策缩小 DS-O1 的待决范围，但不自动关闭其余页面的视觉变化范围决策。详细规则见 `docs/design-system/HOMEPAGE_VISUAL_FREEZE.md`。

处理规则：关闭任一项目时，必须记录用户决策、影响阶段和验证要求，并同步更新本决策组的整体状态。
